...
<section
	...
	aria-label="<?php esc_attr_e('Charge session history', 'gaming-hub'); ?>"
	data-tesla-charge
...
>
	<div class="ecoflow-plan-header ecoflow-plan-head">
		...
		<p class="ecoflow-plan-limits">
			...
			<span class="ecoflow-cal-nav">
				<button type="button" class="ecoflow-plan-cancel" data-tesla-charge-prev data-month="<?php echo esc_attr( $log['prev'] ?? '' ); ?>"><?php esc_html_e('Prev month', 'gaming-hub'); ?></button>
				<button type="button" class="ecoflow-plan-cancel" data-tesla-charge-next data-month="<?php echo esc_attr( $log['next'] ?? '' ); ?>"><?php esc_html_e('Next month', 'gaming-hub'); ?></button>
			</span>
		</p>
	</div>
	...
	<div class="ecoflow-rates-hud ecoflow-plan-hud ecoflow-cal-hud">
		...
		<div class="ecoflow-rates-stat">
			<span><?php esc_html_e('Sessions this month', 'gaming-hub'); ?></span>
			...
			<small data-tesla-charge-count-detail>
				...
				printf(
					...
					esc_html__('Home %1$s · Super %2$s', 'gaming-hub'),
		...
		<div class="ecoflow-rates-stat">
			<span><?php esc_html_e('Energy this month', 'gaming-hub'); ?></span>
		...
		<div class="ecoflow-rates-stat ecoflow-cal-stat-buy">
			<span><?php esc_html_e('Cost this month', 'gaming-hub'); ?></span>
	...
	<ol class="tesla-charge-list" data-tesla-charge-list>
		<?php if ( $current ) : ?>
			<li class="tesla-charge-row is-active<?php echo 'supercharger' === ( $current['supply'] ?? '' ) ? ' is-super' : ''; ?>" data-tesla-charge-current>
				<div class="tesla-charge-when">
					<strong><?php echo esc_html( $current['when_label'] ?: __('Charging', 'gaming-hub') ); ?></strong>
					<span class="tesla-charge-badge"><?php esc_html_e('In progress', 'gaming-hub'); ?></span>
					<span class="tesla-charge-supply"><?php echo esc_html( $current['supply_label'] ?? __('Home charging', 'gaming-hub') ); ?></span>
		...
		<?php if ( empty( $sessions ) && ! $current ) : ?>
			<li class="tesla-charge-empty" data-tesla-charge-empty>
				<?php esc_html_e('No charge sessions this month yet. They appear after the next home/Supercharger session or a Fleet history sync.', 'gaming-hub'); ?>
			</li>
		<?php endif; ?>
		...
		<?php foreach ( $sessions as $session ) : ?>
			<li class="tesla-charge-row<?php echo 'supercharger' === ( $session['supply'] ?? '' ) ? ' is-super' : ''; ?>">
				<div class="tesla-charge-when">
					...
					<span class="tesla-charge-supply"><?php echo esc_html( $session['supply_label'] ?? __('Home charging', 'gaming-hub') ); ?></span>
					...
					<?php if ( ! empty( $session['limit_soc'] ) ) : ?>
						<span class="tesla-charge-limit"><?php echo esc_html( sprintf( __('Limit %s%%', 'gaming-hub'), (string) (int) $session['limit_soc'] ) ); ?></span>
					<?php endif; ?>
				</div>
				<div class="tesla-charge-meta">
