...
/** Tesla GPS fix minus geofence centre (set by one-tap home calibration). */
define( 'GAMING_HUB_TESLA_GPS_OFFSET_OPTION', 'gaming_hub_tesla_gps_offset_v1' );
/** Default home: 多治見市脇之島町6-47-4 付近（旧 35.3409/137.1264 は上野町側で約 2 km ずれ）。 */
define( 'GAMING_HUB_TESLA_HOME_LAT_DEFAULT', 35.3178 );
define( 'GAMING_HUB_TESLA_HOME_LON_DEFAULT', 137.1142 );
define( 'GAMING_HUB_TESLA_HOME_LAT_LEGACY', 35.3409 );
...
/**
 * One-time: move legacy home coords (上野町側) to 脇之島町6-47-4 付近.
 */
function gaming_hub_bump_tesla_home_coords_v2() {
	if ( get_option( 'gaming_hub_tesla_home_coords_bump_v2' ) ) {
		return;
	}

	$lat = (float) get_theme_mod( 'tesla_home_lat', GAMING_HUB_TESLA_HOME_LAT_LEGACY );
	$lon = (float) get_theme_mod( 'tesla_home_lon', GAMING_HUB_TESLA_HOME_LON_LEGACY );
	if (
		abs( $lat - GAMING_HUB_TESLA_HOME_LAT_LEGACY ) < 0.00001
		&& abs( $lon - GAMING_HUB_TESLA_HOME_LON_LEGACY ) < 0.00001
	) {
		set_theme_mod( 'tesla_home_lat', GAMING_HUB_TESLA_HOME_LAT_DEFAULT );
		set_theme_mod( 'tesla_home_lon', GAMING_HUB_TESLA_HOME_LON_DEFAULT );
	}

	update_option( 'gaming_hub_tesla_home_coords_bump_v2', 1 );
}
add_action( 'init', 'gaming_hub_bump_tesla_home_coords_v2', 20 );
...
/**
 * One-time: drop legacy street address from the home geofence label.
 */
function gaming_hub_bump_tesla_home_name_v1() {
	if ( get_option( 'gaming_hub_tesla_home_name_bump_v1' ) ) {
		return;
	}
...
add_action( 'init', 'gaming_hub_bump_tesla_home_name_v1', 20 );
...
/**
 * Saved Tesla GPS calibration offset (raw fix + offset ≈ home centre).
 *
 * @return array{lat: float, lon: float}|null
 */
function gaming_hub_tesla_gps_offset() {
	$saved = get_option( GAMING_HUB_TESLA_GPS_OFFSET_OPTION, null );
...
/**
 * Apply home GPS calibration to a raw vehicle fix.
 *
 * @param array{lat: float, lon: float} $coords Raw coordinates.
 * @return array{lat: float, lon: float}
 */
function gaming_hub_tesla_apply_gps_offset( array $coords ) {
	$offset = gaming_hub_tesla_gps_offset();
	if ( ! $offset ) {
		return $coords;
	}

	return array(
		'lat' => (float) $coords['lat'] + (float) $offset['lat'],
		'lon' => (float) $coords['lon'] + (float) $offset['lon'],
	);
}

/**
 * Calibrate GPS offset from the last raw Tesla fix while parked at home.
 *
 * @return array<string, mixed>|WP_Error
 */
function gaming_hub_tesla_calibrate_gps_offset() {
	...
	$offset = array(
		'lat' => (float) $home['lat'] - (float) $saved['lat'],
		'lon' => (float) $home['lon'] - (float) $saved['lon'],
		'at'  => time(),
	);
	update_option( GAMING_HUB_TESLA_GPS_OFFSET_OPTION, $offset, false );
	...
	return array(
		'offset_lat' => round( $offset['lat'], 6 ),
		'offset_lon' => round( $offset['lon'], 6 ),
		'home_lat'   => (float) $home['lat'],
		'home_lon'   => (float) $home['lon'],
		'tesla_lat'  => (float) $saved['lat'],
		'tesla_lon'  => (float) $saved['lon'],
	);
}
...
/**
 * Whether a reverse-geocoded label is within 多治見市 (treated as home).
 *
 * @param string $place Reverse-geocoded place label.
 */
function gaming_hub_tesla_place_is_tajimi( $place ) {
	$place = trim( (string) $place );
	if ( '' === $place ) {
		return false;
	}

	return false !== mb_strpos( $place, '多治見', 0, 'UTF-8' )
		|| false !== stripos( $place, 'Tajimi' );
}

/**
 * Whether vehicle coordinates reverse-geocode to 多治見市.
 *
 * @param float $lat Latitude.
 * @param float $lon Longitude.
 */
function gaming_hub_tesla_coords_in_tajimi( $lat, $lon ) {
	if ( ! function_exists( 'gaming_hub_tesla_reverse_geocode' ) ) {
		return false;
	}

	return gaming_hub_tesla_place_is_tajimi(
		gaming_hub_tesla_reverse_geocode( (float) $lat, (float) $lon )
	);
}

/**
 * Whether coordinates are within home AC territory (geofence circle or 多治見市).
 *
 * @param float $lat Latitude.
 * @param float $lon Longitude.
 * @return bool|null Null when coords are invalid.
 */
function gaming_hub_tesla_coords_at_home_ac( $lat, $lon ) {
	$lat = (float) $lat;
	$lon = (float) $lon;
	...
	if ( $dist <= (float) $home['radius_m'] ) {
		return true;
	}

	return gaming_hub_tesla_coords_in_tajimi( $lat, $lon );
}

/**
 * Capture GPS for a charge session start (stored on the energy option row).
 *
 * @param array<string, mixed> $meta Optional lat/lon or live at_home fallback.
 * @return array<string, mixed>|null
 */
function gaming_hub_tesla_charge_gps_capture( array $meta = array() ) {
	$lat = null;
	$lon = null;

	if ( isset( $meta['lat'], $meta['lon'] ) && is_numeric( $meta['lat'] ) && is_numeric( $meta['lon'] ) ) {
		$lat = (float) $meta['lat'];
		$lon = (float) $meta['lon'];
	} else {
		$saved = get_option( GAMING_HUB_TESLA_LAST_COORD_OPTION, null );
	...
	if ( null === $lat || null === $lon ) {
		if ( array_key_exists( 'at_home', $meta ) && null !== $meta['at_home'] ) {
			return array(
				'lat'         => null,
				'lon'         => null,
				'at_home_ac'  => (bool) $meta['at_home'],
				'captured_at' => time(),
				'source'      => 'at_home_fallback',
			);
		}
	...
	$coords     = gaming_hub_tesla_apply_gps_offset(
		array(
			'lat' => $lat,
			'lon' => $lon,
		)
	);
	$at_home_ac = gaming_hub_tesla_coords_at_home_ac( $coords['lat'], $coords['lon'] );
	if ( null === $at_home_ac && array_key_exists( 'at_home', $meta ) && true === $meta['at_home'] ) {
		$at_home_ac = true;
	}
...
/**
 * Plan chart / history input key from supply + saved session GPS.
 *
 * @param string                    $supply home|supercharger.
 * @param array<string, mixed>|null $gps    Session GPS snapshot.
 * @return string home_ac|away_ac|dc
 */
function gaming_hub_tesla_session_charge_input( $supply, $gps = null ) {
	if ( 'supercharger' === $supply ) {
		return 'dc';
	}

	if ( is_array( $gps ) && array_key_exists( 'at_home_ac', $gps ) ) {
		return ! empty( $gps['at_home_ac'] ) ? 'home_ac' : 'away_ac';
	}
 ...
 *
 * @param array<string, mixed> $data vehicle_data.
 * @return array{at_home: bool|null, at_home_geofence: bool|null, distance_m: int|null, known: bool}
 */
...
/**
 * Whether the sticky at-home flag is active.
 */
function gaming_hub_tesla_at_home_sticky_active() {
	$saved = get_option( GAMING_HUB_TESLA_AT_HOME_STICKY_OPTION, null );

	return is_array( $saved ) && ! empty( $saved['active'] );
}

/**
 * Remember a GPS-confirmed home geofence hit.
 */
function gaming_hub_tesla_at_home_sticky_mark() {
	update_option(
		GAMING_HUB_TESLA_AT_HOME_STICKY_OPTION,
		array(
			'active' => true,
			'at'     => time(),
		),
...
/**
 * Drop sticky at-home after drive, Supercharger, or a confirmed away fix.
 */
function gaming_hub_tesla_at_home_sticky_clear() {
	delete_option( GAMING_HUB_TESLA_AT_HOME_STICKY_OPTION );
}
...
function gaming_hub_tesla_at_home_resolve( array $geofence, $moving, $supercharger, $ac_plugged = false ) {
	if ( $moving || $supercharger ) {
		...
		return array(
			'at_home'             => ! empty( $geofence['known'] ) ? $geofence['at_home'] : false,
	...
	if ( gaming_hub_tesla_at_home_sticky_active() ) {
		return array(
			'at_home'             => true,
			'geofence_known'      => false,
...
/**
 * Whether a cached Model 3 snapshot indicates driving.
 *
 * @param array<string, mixed> $model3 Cached payload.
 */
function gaming_hub_tesla_snapshot_is_moving( array $model3 ) {
	if ( empty( $model3['drive_ready'] ) ) {
		return false;
	}
...
/**
 * Whether a cached Model 3 snapshot is at a Supercharger stall.
 *
 * @param array<string, mixed> $model3 Cached payload.
 */
function gaming_hub_tesla_snapshot_is_supercharger( array $model3 ) {
	if ( ! empty( $model3['fast_charger_present'] ) ) {
		return true;
	}

	return 'supercharger' === (string) ( $model3['supply_kind'] ?? '' );
}
 ...
 * @param bool|null                                                    $at_home    Resolved at-home state.
 * @return string
 */
...
function gaming_hub_tesla_model3_input_at_home( array $model3 ) {
	...
	if ( ! empty( $model3['geofence_known'] ) && array_key_exists( 'at_home', $model3 ) ) {
		return true === $model3['at_home'];
	}

	return array_key_exists( 'at_home', $model3 ) ? $model3['at_home'] : null;
}
...
function gaming_hub_tesla_model3_record_odometer( $odometer_km ) {
	...
	$advance_last_ts = $new_day || $delta_km > 0 || $last_ts <= 0;
	$candidate_km    = null;
	$candidate_hits  = 0;
	...
	if ( $delta_km > 0 ) {
		...
		if ( $reject && null !== $last_km ) {
			...
			$candidate_hits  = ( null !== $prev_candidate && abs( $prev_candidate - $odometer_km ) <= 1.0 )
				? max( 1, $prev_hits ) + 1
				: 1;
			$odometer_km     = $last_km;
			$today_km        = max( 0, round( $odometer_km - $start_km, 1 ) );
			$delta_km        = 0.0;
			$advance_last_ts = false;
		} else {
	...
	$option = array(
		...
		'wh'             => $wh,
		'yen'            => $yen,
		'last_ts'        => $advance_last_ts ? $now : ( $last_ts > 0 ? $last_ts : $now ),
		'updated_at'     => $now,
	);
	if ( null !== $candidate_km ) {
		$option['candidate_km']   = $candidate_km;
		$option['candidate_hits'] = $candidate_hits;
	}

	update_option( GAMING_HUB_MODEL3_ODO_OPTION, $option, false );
...
function gaming_hub_tesla_model3_from_vehicle_data( array $data ) {
	...
	return gaming_hub_powerwall_model3_present(
		array(
			...
			'at_home'                   => $at_home,
			'at_home_geofence'          => ! empty( $geofence['known'] )
				? (bool) ( $geofence['at_home_geofence'] ?? false )
				: null,
			'geofence_known'            => ! empty( $home_ctx['geofence_known'] ),
			'geofence_distance_m'       => $home_ctx['geofence_distance_m'],
			'at_home_sticky'            => ! empty( $home_ctx['at_home_sticky'] ),
			'location_debug'            => '',
...
function gaming_hub_register_tesla_rest_routes() {
	...
	register_rest_route(
		'gaming-hub/v1',
		'/tesla/calibrate-home-gps',
		array(
			'methods'             => 'POST',
			'callback'            => 'gaming_hub_rest_tesla_calibrate_home_gps',
			'permission_callback' => 'gaming_hub_tesla_can_control',
		)
	);
}
...
/**
 * REST: POST /gaming-hub/v1/tesla/calibrate-home-gps
 *
 * @return WP_REST_Response|WP_Error
 */
function gaming_hub_rest_tesla_calibrate_home_gps() {
	$result = gaming_hub_tesla_calibrate_gps_offset();
	if ( is_wp_error( $result ) ) {
		return new WP_REST_Response(
			array(
				'success' => false,
