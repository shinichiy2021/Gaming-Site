...
import { analyzeStockPortfolio, type StockIdeaItem } from '@/lib/stockAnalysis';
...
const priorityLabel: Record<StockIdeaItem['priority'], string> = {
  now: '今すぐ',
  watch: '様子見',
  setup: 'セットアップ',
};
...
export default function StockPortfolioInsight({ portfolio, loading, refreshing, onRefresh }: Props) {
  const insight = useMemo(() => analyzeStockPortfolio(portfolio), [portfolio]);
  const busy = loading || Boolean(refreshing);

  return (
    <section
      className={`grid gap-4 lg:grid-cols-2 ${busy ? 'opacity-80' : ''}`}
      aria-labelledby="stock-insight-title"
    >
      <div className="rounded-2xl border border-[var(--color-line)] bg-[var(--color-panel)] p-5 backdrop-blur-md">
        ...
        {insight.hasHoldings ? (
          <div className="mb-4 flex flex-wrap gap-2">
            ...
            {insight.top ? (
              <span className="rounded-full border border-white/15 px-2.5 py-1 text-[0.7rem] text-[var(--color-muted)]">
                Top {insight.top.code || insight.top.name.slice(0, 6)} {insight.top.weight}%
              </span>
            ) : null}
            <span className="rounded-full border border-white/15 px-2.5 py-1 text-[0.7rem] text-[var(--color-muted)]">
      ...
      <div className="rounded-2xl border border-[var(--color-line)] bg-[var(--color-panel)] p-5 backdrop-blur-md">
        ...
        <p className="mb-4 text-xs text-[var(--color-muted)]">
          前日比・含み損益・配分から自動生成。CSV再取込や再取得で金額・％を更新します
